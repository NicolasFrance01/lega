...
async function migrate() {
    ...
    const sheetName = 'Federacion-PAMI';
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        else if (firstCell === "Prestaciones" || firstCell === "Nombre" || (row && row.includes("UB"))) metaPart = "HEADER";
        else if (firstCell.includes("NOTA:")) metaPart = "NOTE";
        
        rowData["meta_part"] = metaPart;

        // Procesar celdas (usar max columns para consistencia)
        const maxCols = 13; 
        ...
        if (metaPart === "TITLE") rowData["__row_color"] = "#e8f1ff";
        if (metaPart === "HEADER") {
           Object.keys(rowData).forEach(k => {
               if (k.startsWith("__EMPTY")) rowData[`__cell_color_${k}`] = "#e8f1ff";
           });
        }
