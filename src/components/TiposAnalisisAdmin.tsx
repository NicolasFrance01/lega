...
import { createTipoAnalisis, updateTipoAnalisis, deleteTipoAnalisis } from "@/actions/listados";

export default function TiposAnalisisAdmin({ initialData }: { initialData: any[] }) {
  ...
  async function handleCreate() {
    ...
    setLoading(true);
    const res = await createTipoAnalisis(newNombre);
  ...
  async function handleUpdate(id: number) {
    setLoading(true);
    const res = await updateTipoAnalisis(id, editNombre, editActivo);
    if (res.error) { setError(res.error); }
    else {
  ...
  async function handleDelete(id: number, nombre: string) {
    if (!confirm(`¿Eliminar el análisis "${nombre}"?`)) return;
    const res = await deleteTipoAnalisis(id);
    if (res.error) { setError(res.error); }
